 ...
 */
 // 原厂硬件标定容量 mAh
#define RAW_DESIGN_CAP_MAH 4000
...
struct bq_fg_chip {
	...
	int fake_temp;
	int fake_volt;
	/* 新增：SOC平滑防抖状态 移植自battery‑simulator */
 	int smooth_current_soc;
 	int smooth_target_soc;
 	int stable_target_cnt;
...
static int fg_read_rsoc(struct bq_fg_chip *bq)
{
    ...
    regmap_read(bq->regmap, bq->regs[BQ_FG_REG_SOC], &soc_raw_reg);
    fg_read_word(bq, bq->regs[BQ_FG_REG_VOLT], &real_volt);
    fg_read_current(bq, &batt_current);

    rm_scaled = fg_read_rm(bq);
    fcc_scaled = fg_read_fcc(bq);

    if (fcc_scaled <= 0)
        soc_coulomb = 50;
    else
        soc_coulomb = DIV_ROUND_CLOSEST(rm_scaled * 100, fcc_scaled);
    soc_coulomb = clamp(soc_coulomb, 0, 100);

    soc_volt = bq_voltage_to_soc(real_volt);

    if (batt_current < 0) {
        /* 充电：信任库仑计 */
        final_soc = soc_coulomb;
        if (++soc_fix_log_cnt % 20 == 0) {
            bq_dbg(PR_OEM, "CHARGE MODE, coulomb soc:%d, volt soc:%d, volt:%dmV\n",
                soc_coulomb, soc_volt, real_volt);
        }
    } else {
        /* 放电：库仑计70% + 电压30%加权兜底 */
        u32 coulomb_term = (soc_coulomb + 20) * 20;
        u32 volt_term = (soc_volt + 8) * 80;
    ...
    /* !!! 删除原来这里 static old_soc / delta ±2 的全部旧截断代码，平滑交给bq_battery_soc_smooth_tracking !!! */
    return final_soc;
}
...
static int fg_read_fcc(struct bq_fg_chip *bq)
{
    ...
    ret = fg_read_word(bq, bq->regs[BQ_FG_REG_FCC], &fcc_raw);
    if (ret < 0) {
        bq_dbg(PR_OEM, "could not read FCC, ret=%d\n", ret);
        fcc_raw = RAW_DESIGN_CAP_MAH;
    }
    // 统一倍率缩放
    return DIV_ROUND_CLOSEST(fcc_raw * scale, 1000);
}
...
static int fg_read_rm(struct bq_fg_chip *bq)
{
    ...
    ret = fg_read_word(bq, bq->regs[BQ_FG_REG_RM], &rm_raw);
    if (ret < 0) {
    ...
    // 容量限幅保护
    if (rm_scaled <= 0)
        rm_scaled = max_cap / 10;
    if (rm_scaled > max_cap)
        rm_scaled = max_cap;
    return rm_scaled;
}
					...
					union power_supply_propval *val)
{
	...
	if (bq->charge_full_design_override) {
        val->intval = bq->custom_charge_full_design;
        break;
    }
...
/**
 * is_target_stable：判断目标是否连续N次采样保持同一个值
 * return 1=稳定允许更新; 0=抖动暂不更新
 */
static int is_target_stable(struct bq_fg_chip *bq, int target, int threshold_cnt)
{
	if (target == bq->smooth_target_soc) {
		bq->stable_target_cnt++;
	} else {
		bq->stable_target_cnt = 1;
		bq->smooth_target_soc = target;
	}
...
static int bq_battery_soc_smooth_tracking(struct bq_fg_chip *bq,
		int raw_soc, int batt_soc, int temp, int batt_ma)
{
#define SMOOTH_STEP		1
#define STABLE_THRESHOLD	3

	/* 首次初始化 */
	if (bq->smooth_current_soc < 0) {
		bq->smooth_current_soc = batt_soc;
		bq->smooth_target_soc = batt_soc;
		bq->stable_target_cnt = 0;
		return clamp(batt_soc, 0, 100);
	}

	if (batt_ma < 0) {
		/* 充电：不做稳定计数，直接平滑追赶 */
		smooth_transition(batt_soc, &bq->smooth_current_soc, SMOOTH_STEP);
		bq->stable_target_cnt = 0;
	} else {
		/* 放电：目标必须连续多次相同才允许向下更新，过滤瞬时负载毛刺 */
		if (is_target_stable(bq, batt_soc, STABLE_THRESHOLD)) {
			smooth_transition(batt_soc, &bq->smooth_current_soc, SMOOTH_STEP);
		}
	}

	return clamp(bq->smooth_current_soc, 0, 100);
}
				...
				const struct i2c_device_id *id)
{
	...
	bq->fake_volt	= -EINVAL;
	/* === 新增平滑状态初始化 === */
	bq->smooth_current_soc = -1;
	bq->smooth_target_soc = 0;
	bq->stable_target_cnt = 0;
	/* 自定义容量覆盖变量初始化 */
